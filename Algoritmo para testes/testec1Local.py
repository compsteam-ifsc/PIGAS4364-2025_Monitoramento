...
class ApiClient:
    ...
    def enviar(self, tipo, retry=True):
        ...
        try:
            ...
            log.error("Erro ao enviar %s: %s", tipo, e)
            return

        log.info("%s -> status %s", tipo, r.status_code)
        if r.status_code == 401 and retry:
            ...
            try:
                ...
                return
            self.enviar(tipo, retry=False)
        elif r.status_code not in (200, 201):
            log.error("Erro no backend: %s", r.text)
    ...
    def _salvar(self):
        ...
        try:
            with open(tmp, "w", encoding="utf-8") as f:
                json.dump(sorted(self._itens), f)
