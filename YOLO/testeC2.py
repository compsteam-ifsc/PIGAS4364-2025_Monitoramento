...
INVERTER = True  # mantém sua lógica original
# ================================================
...
def obter_token():
    ...
    if resp.status_code == 200:
        data = resp.json()
        print(f"[AUTH] OK | role: {data.get('role')}")
        return data.get("token")

    raise RuntimeError(f"[AUTH ERRO] {resp.status_code}: {resp.text}")


def enviar(tipo, token, retry=True):
    global _token
    ...
    try:
        ...
        print(f"[API] {tipo} -> {r.status_code}")
